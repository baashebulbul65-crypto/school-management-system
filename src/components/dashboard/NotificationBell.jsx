      ...
      {isOpen && (
        <div className="nb-panel">
          <div className="nb-panel-head">
            ...
            {unreadCount > 0 && (
              <button className="nb-mark-all" onClick={markAllAsRead}>{t('notifications.markAllRead')}</button>
            )}
          </div>

          <div className="nb-list">
            {recent.length === 0 && <div className="nb-empty">{t('notifications.empty')}</div>}
            {recent.map((n) => {
          ...
          <button className="nb-view-all" onClick={handleViewAll}>{t('notifications.viewAll')}</button>
        </div>
      )}
    </div>
  );
